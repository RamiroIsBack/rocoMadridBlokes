                    ...
                    if (posts && posts.length > 0) {
                        ...
                        const parsedCards = posts.map((post) => {
                            ...
                            return {
                                ...
                                title: ((post.title && post.title.rendered) || 'Sin título').slice(0, 30),
                                category,
                                sala,
                                subsala,
                                tipo,
    ...
    return {
        ...
        refresh: useCallback(() => {
            ...
            setTimeout(() => {
                ...
                if (hasCredentials) {
                    fetch(`${WORDPRESS_URL}/wp-json/wp/v2/blokes?` + new URLSearchParams({
                            per_page: 100,
                            _embed: '',
                            acf_format: 'standard',
                        }), {
                        ...
                        .then(posts => {
                            if (posts && posts.length > 0) {
                                const parsedCards = posts.map((post) => {
                                    ...
                                    if (acf.bloke_gallery) {
                                        acf.bloke_gallery.forEach(att => {
                                            if (att && att.url) {
                                                images.push({
                                                    url: att.url,
                                                    isVideo: att.is_video || false,
                                                    id: att.id || 0
                                                })
                                            }
                                        })
                                    ...
                                    return {
                                        ...
                                        title: ((post.title && post.title.rendered) || 'Sin título').slice(0, 30),
                                        category,
                                        sala: acf.bloke_sala || 'entrada',
                                        subsala: acf.bloke_subsala || '1',
...
export async function deleteBloke(postId) {
    ...
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({}))
            throw new Error(errorData.message || 'Failed to delete bloke')
        }
 ...
 * @param {number} postId - The ID of the bloke to update
 * @param {Object} data - The bloke data to update
 * @returns {Promise<Object>}
 */
export async function updateBloke(postId, data) {
    ...
    try {
        ...
        if (!response.ok) {
            const errorData = await response.json().catch(() => ({}))
            throw new Error(errorData.message || 'Failed to update bloke')
        }

        return await response.json()
    } catch (err) {
        console.error('Error updating bloke:', err)
        throw err
    }
}